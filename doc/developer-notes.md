...
Run with the -testnet option to run with "play dietbitcoins" on the test network, if you
...
DietBitcoin Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- DietBitcoinMiner : Generates dietbitcoins (if wallet is enabled).
...
However, in open source software such as DietBitcoin Core, where everyone uses
...
pay attention to for reviewers of DietBitcoin Core code.

General DietBitcoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: DietBitcoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of DietBitcoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against DietBitcoin Core so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:dietbitcoin/dietbitcoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `dietbitcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `dietbitcoin-cli` and the GUI debug console use this table to determine how to
