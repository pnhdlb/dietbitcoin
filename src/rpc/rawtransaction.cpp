...
{
    // Call into TxToUniv() in dietbitcoin-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in dietbitcoin-common, so we query them here and push the
...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx,    (numeric or string, required) The key is the dietbitcoin address, the numeric value (can be string) is the " +
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            CURRENCY_UNIT + "\n"
                            ...
                            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
                            "         \"addresses\" : [           (json array of string)\n"
                            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) dietbitcoin address\n"
                            "           ,...\n"
                            "         ]\n"
                            "       }\n"
                            "     }\n"
...
UniValue decodescript(const JSONRPCRequest& request)
{
    ...
    if (type.isStr() && type.get_str() != "scripthash") {
        ...
        r.push_back(Pair("p2sh", CDietBitcoinAddress(CScriptID(script)).ToString()));
    }
