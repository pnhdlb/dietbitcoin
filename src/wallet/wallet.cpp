...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    if (redeemScript.size() > MAX_SCRIPT_ELEMENT_SIZE) {
        std::string strAddr = CDietBitcoinAddress(CScriptID(redeemScript)).ToString();
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/dietbitcoin/dietbitcoin/pull/10340, but that change has been
    // postponed until after https://github.com/dietbitcoin/dietbitcoin/pull/10244 to
...
bool CWallet::CreateTransaction(const std::vector<CRecipient>& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-dietbitcoin-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(*dbw).WritePurpose(CDietBitcoinAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(*dbw).WriteName(CDietBitcoinAddress(address).ToString(), strName);
}
...
{
    {
        ...
        std::string strAddress = CDietBitcoinAddress(address).ToString();
    ...
    CWalletDB(*dbw).ErasePurpose(CDietBitcoinAddress(address).ToString());
    return CWalletDB(*dbw).EraseName(CDietBitcoinAddress(address).ToString());
}
 ...
 * https://dietbitcointalk.org/?topic=54527, or
 * https://github.com/dietbitcoin/dietbitcoin/pull/1393.
 */
...
bool CWallet::AddDestData(const CTxDestination& dest, const std::string& key, const std::string& value)
{
    ...
    return CWalletDB(*dbw).WriteDestData(CDietBitcoinAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination& dest, const std::string& key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return CWalletDB(*dbw).EraseDestData(CDietBitcoinAddress(dest).ToString(), key);
}
