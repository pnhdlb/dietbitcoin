...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help")) {
        ...
        std::string strUsage = strprintf(_("%s dietbitcoin-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
                               _("Usage:") + "\n" +
                               "  dietbitcoin-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded dietbitcoin transaction") + "\n" +
                               "  dietbitcoin-tx [options] -create [commands]   " + _("Create hex-encoded dietbitcoin transaction") + "\n" +
                               "\n";
...
static void MutateTxAddOutAddr(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CDietBitcoinAddress addr(strAddr);
    if (!addr.IsValid())
...
static void MutateTxAddOutPubKey(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    if (bScriptHash) {
        ...
        CDietBitcoinAddress redeemScriptAddr(scriptPubKey);
...
static void MutateTxAddOutMultiSig(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    if (bScriptHash) {
        // Get the address for the redeem script, then call
        // GetScriptForDestination() to construct a P2SH scriptPubKey.
        CDietBitcoinAddress addr(scriptPubKey);
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded dietbitcoin transaction
